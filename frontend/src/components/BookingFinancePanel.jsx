...
export function BookingFinancePanel({
  ...
  return (
    <div>
      ...
      {canViewInvoices && invoices.length === 0 && <p className="muted" style={{ fontSize: 13 }}>{t('noInvoiceYet')}</p>}
        ...
        <div key={inv.id} style={{ border: '1px solid var(--color-border,#e5e7eb)', borderRadius: 10, padding: 12, marginBottom: 10 }}>
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
              {canCancel && inv.status === 'issued' && (
                <button className="btn btn-ghost btn-sm" disabled={busy} style={{ color: 'var(--color-danger,#dc2626)' }}
                  onClick={() => onCancel?.(inv)}><Ban size={13} /> {t('common:actions.cancel')}</button>
              )}
              {canReverse && ['paid', 'partially_refunded'].includes(inv.status) && (
                <button className="btn btn-ghost btn-sm" disabled={busy} style={{ color: 'var(--color-warning,#d97706)' }}
                  onClick={() => onRefund?.(inv)}><RotateCcw size={13} /> {t('refund')}</button>
              )}
            </div>
          </div>
          <div className="form-grid form-grid--2" style={{ gap: '2px 16px', fontSize: 13 }}>
            <span className="muted">{t('invoiceDate')}</span><span style={{ textAlign: 'right' }}>{dt(inv.issued_at)}</span>
            <span className="muted">{t('common:labels.amount')}</span><span style={{ textAlign: 'right' }}><Money amount={inv.total} code={inv.currency} /></span>
            <span className="muted">{t('outstanding')}</span><span style={{ textAlign: 'right' }}><Money amount={inv.outstanding} code={inv.currency} /></span>
            {Number(inv.refunded_total) > 0 && (<><span className="muted">{t('refunded')}</span><span style={{ textAlign: 'right' }}><Money amount={inv.refunded_total} code={inv.currency} /></span></>)}
            <span className="muted">{t('billed')}</span><span style={{ textAlign: 'right' }}>{inv.bill_to || '-'}</span>
          </div>
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            <button className="btn btn-secondary btn-sm" onClick={() => onDownloadInvoice?.(inv)}><FileText size={13} /> {t('invoicePdf')}</button>
            {inv.receipt && <button className="btn btn-secondary btn-sm" onClick={() => onDownloadReceipt?.(inv)}><FileText size={13} /> {t('receiptPdf')}</button>}
          </div>
          ...
          {(inv.credit_notes || []).length > 0 && (
            <div style={{ marginTop: 10, borderTop: '1px dashed var(--color-border,#e5e7eb)', paddingTop: 8 }}>
              <div className="muted" style={{ fontSize: 11.5, textTransform: 'uppercase', letterSpacing: '.04em', fontWeight: 700, marginBottom: 4 }}>{t('returns')}</div>
              {inv.credit_notes.map((cn) => (
                <div key={cn.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, padding: '3px 0' }}>
                  ...
                  <span style={{ marginLeft: 'auto' }}>{t('returned')} <strong><Money amount={cn.total} code={cn.currency || inv.currency} /></strong></span>
                  <button className="btn btn-ghost btn-sm" title={t('creditNotePdf')} onClick={() => onDownloadCreditNote?.(cn)}>
                    <FileText size={13} />
                  </button>
                </div>
              ))}
      ...
      {canViewInvoices && onGenerate && !liveInvoice && (
        <button className="btn btn-ghost btn-sm" disabled={busy} onClick={onGenerate}>
          <FileText size={15} /> {invoices.length ? t('reissueInvoice') : t('generateInvoice')}
        </button>
      )}
      ...
      {canViewInvoices && onCollect && liveInvoice && outstanding > 0 && (
        <button className="btn btn-secondary btn-sm" disabled={busy} onClick={onCollect}>
          <FileText size={15} /> {t('collectAdditionalPayment')} (<Money amount={outstanding} code={currency} />)
        </button>
      )}
        ...
        <>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr>
                <th style={head}>{t('dateAndTime')}</th><th style={head}>{t('method')}</th><th style={head}>{t('reference')}</th>
                <th style={{ ...head, textAlign: 'right' }}>{t('common:labels.amount')}</th><th style={head}>{t('common:labels.status')}</th><th style={head}>{t('created')}</th>
                      ...
                      {(() => {
                        ...
                        return refundable ? (
                          <button className="btn btn-ghost btn-sm" disabled={busy}
                            ...
                            onClick={() => onRefund?.(target)}><RotateCcw size={13} /> {t('refund')}</button>
                        ) : null;
                      })()}
                    </td>
                  </tr>
      ...
      {canViewInvoices && refunds.length > 0 && (
        <>
          <SectionLabel>{t('refunds')}</SectionLabel>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr>
                <th style={head}>{t('common:labels.date')}</th><th style={head}>{t('reference')}</th><th style={head}>{t('origPayment')}</th>
                <th style={{ ...head, textAlign: 'right' }}>{t('common:labels.amount')}</th><th style={head}>{t('method')}</th>
                <th style={head}>{t('common:labels.status')}</th><th style={head}>{t('common:labels.reason')}</th><th style={head}>{t('processed')}</th>
              </tr></thead>
              <tbody>
                {refunds.map((r) => (
