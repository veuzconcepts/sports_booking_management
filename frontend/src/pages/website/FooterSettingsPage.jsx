...
export default function FooterSettingsPage() {
  ...
  useEffect(() => {
    footerApi.get()
      .then(setCfg)
      .catch((e) => toast.error(apiErrorMessage(e, 'Unable to load the footer settings.')));
  }, []);
  ...
  async function save() {
    ...
    try {
      ...
      toast.error(apiErrorMessage(e, 'Unable to save the footer. Please try again.'));
  ...
  return (
    <>
      <PageHeader title="Footer" subtitle="Footer content for the customer website."
        actions={canEdit && (
      ...
      <div className="card" style={{ padding: 18, maxWidth: 820 }}>
        <fieldset disabled={!canEdit} style={{ border: 0, padding: 0, margin: 0 }}>
            ...
            <div key={ci} style={{ border: '1px solid var(--color-border)', borderRadius: 10, padding: 12, marginBottom: 10 }}>
              ...
              {(col.links || []).map((lnk, li) => (
                <div key={li} style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                  ...
                  <button type="button" className="icon-btn" title="Remove link"
                    onClick={() => {
                      const next = [...columns];
          ...
          <Toggle label="Newsletter enabled" description="Show the email sign-up in the footer."
            checked={Boolean(cfg.newsletter_enabled)} onChange={(e) => set({ newsletter_enabled: e.target.checked })} />
          <FormField label="Newsletter heading">
