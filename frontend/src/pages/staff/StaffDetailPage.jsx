...
import {
  staffApi, transfersApi, EMPLOYMENT_TYPES,
  TRANSFER_SHIFT_OPTIONS, TRANSFER_STATUS_LABELS,
...
export default function StaffDetailPage() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([staffApi.get(id), staffApi.performanceHistory(id)])
      ...
      .catch((e) => toast.error(apiErrorMessage(e, 'Unable to load the staff member. Please try again.')))
  ...
  return (
    <>
        ...
        actions={
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <StatusBadge tone={staff.is_available ? 'success' : 'muted'}
                         label={staff.is_available ? 'Available' : 'Off'} />
            {canTransfer && (
              <button className="btn btn-secondary" onClick={() => setTransferOpen(true)}>
                <ArrowLeft size={15} style={{ transform: 'rotate(180deg)' }} /> Transfer
              </button>
            )}
            {canEdit && (
      ...
      <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid var(--color-border)', marginBottom: 18 }}>
        ...
        {canViewActivity && (
          <button style={tabBtnStyle(tab === 'activity')} onClick={() => setTab('activity')}>Activity Log</button>
        )}
        {canClubHistory && (
          <button style={tabBtnStyle(tab === 'history')} onClick={() => setTab('history')}>Club History</button>
        )}
      </div>
        ...
        <div className="col" style={{ flex: '1 1 300px' }}>
          <div className="card">
            ...
            <div className="card-body">
              <KV icon={Mail} label="Email">{staff.email}</KV>
              <KV icon={Phone} label="Phone">{staff.phone || '-'}</KV>
              <KV label="Employment">{(staff.employment_type || '').replace('_', ' ')}</KV>
              <KV label="Base club">{staff.base_club_name || '-'}</KV>
              <KV label="Skills">{staff.skills || '-'}</KV>
              <KV icon={Star} label="Rating">{Number(staff.rating).toFixed(2)} / 5</KV>
              {staff.hired_on && <KV label="Hired">{new Date(staff.hired_on).toLocaleDateString()}</KV>}
            </div>
          </div>
          ...
          <div className="card">
            ...
            {performance.length === 0 ? (
              <div className="empty"><p>No performance snapshots yet.</p></div>
            ) : (
              <div className="table-wrapper">
                <table className="table">
                  <thead><tr><th>Date</th><th>Jobs</th><th>Rating</th><th>On-time</th></tr></thead>
                  <tbody>
                    {performance.map((p) => (
                      <tr key={p.id}>
...
function ActivityLogTab({ staffId }) {
  ...
  return (
    <DataTable
      ...
      emptyHint="Profile, shift, account, role and assignment changes for this employee will appear here."
      columns={[
        { key: 'when', header: 'When', nowrap: true, render: (r) => formatDateTime(r.created_at) },
        { key: 'action', header: 'Action', render: (r) => ACTIVITY_LABELS[r.event] || r.event || '-' },
        { key: 'by', header: 'By', render: (r) => actorLabel(r.actor, r.actor_name, user?.id) },
        { key: 'details', header: 'Details', render: (r) => <ActivityDetails p={r.payload_summary} /> },
        { key: 'source', header: 'Source', render: (r) => MODULE_OF(r.event) },
...
function ReassignRow({ label, options, value, onChange }) {
  return (
    <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 6 }}>
      ...
      <div style={{ width: 240 }}>
        <Select2 options={[{ value: '', label: 'Keep current' }, ...options]}
          value={value} onChange={onChange} placeholder="Keep current" />
      </div>
    </div>
  );
}
...
function TransferModal({ open, staff, onClose, onDone }) {
  ...
  async function submit() {
    ...
    try {
      ...
      toast.error(apiErrorMessage(e, 'Unable to create the transfer. Please try again.'));
    } finally { setBusy(false); }
  }

  return (
    <Modal open={open} onClose={onClose} title="Transfer employee" size="lg"
      footer={(
        <>
      ...
      <div className="row">
        <div className="col"><FormField label="Current club">
          <input className="form-input" value={staff?.base_club_name || '-'} disabled />
        </FormField></div>
        <div className="col"><FormField label="New club *" error={errors.to_club}>
          <Select2 options={clubs.map((s) => ({ value: s.id, label: s.name }))}
            value={form.to_club} onChange={(v) => setField('to_club', v)} placeholder="Choose club…" />
        </FormField></div>
      </div>
      <div className="row">
        <div className="col"><FormField label="Type">
          <Select2 options={[{ value: 'immediate', label: 'Immediate' }, { value: 'scheduled', label: 'Scheduled' }]}
            value={form.transfer_type} onChange={(v) => setField('transfer_type', v)} />
        </FormField></div>
        {scheduled && (
          <div className="col"><FormField label="Effective date *" error={errors.effective_date}>
            <input className="form-input" type="date" value={form.effective_date}
              onChange={(e) => setField('effective_date', e.target.value)} />
          </FormField></div>
        )}
        <div className="col"><FormField label="Shift after transfer">
          <Select2 options={TRANSFER_SHIFT_OPTIONS} value={form.shift_option}
      ...
      {!impact ? (
        ...
        <>
          ...
          {impact.bookings.length > 0 && (
            <p className="muted" style={{ fontSize: 12.5 }}>Keep assignments, or reassign each to another staff member:</p>
          )}
...
function ClubHistoryTab({ staffId, canApprove, canCancel }) {
  ...
  async function approve(transferId) {
    setBusy(true);
    try { await transfersApi.approve(transferId); toast.success('Transfer approved.'); reload(); }
    catch (e) { toast.error(apiErrorMessage(e, 'Unable to approve the transfer. Please try again.')); }
    finally { setBusy(false); }
  }
  async function doCancel() {
    setBusy(true);
    try { await transfersApi.cancel(cancelId); toast.success('Transfer cancelled.'); setCancelId(null); reload(); }
    catch (e) { toast.error(apiErrorMessage(e, 'Unable to cancel the transfer. Please try again.')); }
    finally { setBusy(false); }
  }
  ...
  return (
    <>
      <DataTable
        ...
        onPageChange={(p) => setQuery({ ...query, page: p })}
        emptyTitle="No transfers yet" emptyHint="Club transfers for this employee will appear here."
        columns={[
          { key: 'route', header: 'From → To', render: (r) => `${r.from_club_name || '-'} → ${r.to_club_name || '-'}` },
          { key: 'effective', header: 'Effective', nowrap: true, render: (r) => new Date(r.effective_date).toLocaleDateString() },
          { key: 'reason', header: 'Reason', render: (r) => r.reason || '-' },
          { key: 'status', header: 'Status', render: (r) => <StatusBadge tone={TRANSFER_STATUS_TONE[r.status]} label={TRANSFER_STATUS_LABELS[r.status] || r.status} /> },
          { key: 'by', header: 'Requested by', render: (r) => r.created_by_name || '-' },
              ...
              (r.status === 'pending_approval' || r.status === 'approved') ? (
                <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                  {canApprove && r.status === 'pending_approval' && (
                    <button className="btn btn-primary btn-sm" disabled={busy} onClick={() => approve(r.id)}>Approve</button>
                  )}
                  {canCancel && (
                    <button className="btn btn-ghost btn-sm" disabled={busy} onClick={() => setCancelId(r.id)}>Cancel</button>
                  )}
                </div>
              ) : null
            ),
      ...
      />
      <ConfirmDialog open={Boolean(cancelId)} tone="danger" title="Cancel transfer"
        message="Cancel this club transfer?" confirmLabel="Cancel transfer" busy={busy}
...
function EditStaffModal({ open, staff, onClose, onSaved }) {
  ...
  async function save() {
    ...
    try {
      ...
      toast.error(apiErrorMessage(e, 'Unable to update the staff member. Please try again.'));
    } finally { setBusy(false); }
  }
  ...
  return (
    <Modal open={open} onClose={onClose} title="Edit staff" size="md"
      ...
      <div className="row">
        ...
        <div className="col">
          <FormField label="Employment type">
            <Select2 options={EMPLOYMENT_TYPES} value={form.employment_type}
      ...
      <div className="row">
        <div className="col">
          <FormField label="Base club" hint="Use Transfer Employee to change the club.">
            <input className="form-input" value={staff?.base_club_name || '-'} disabled />
          </FormField>
        </div>
        <div className="col">
          <FormField label="Hired on">
            <input className="form-input" type="date" value={form.hired_on || ''}
              onChange={(e) => set('hired_on', e.target.value)} />
          </FormField>
        </div>
      </div>
      <FormField label="Skills" hint="Comma-separated tags, e.g. ceramic, polish, interior.">
...
function StaffShiftSection({ staff, onSaved, canEdit }) {
  ...
  async function save(week) {
    if (week && Object.keys(week).length) {
      ...
      if (!res.ok) { setErrors(res.errors); toast.error('Fix the highlighted days before saving.'); return; }
    }
    setErrors({});
    setBusy(true);
    try {
      ...
      toast.error(apiErrorMessage(e, 'Unable to save the schedule. Please try again.'));
    } finally { setBusy(false); }
  }
  ...
  return (
    <div className="card">
      <div className="card-header">
        <div>
          <h3 className="card-title"><CalendarClock size={15} style={{ marginRight: 6, verticalAlign: 'text-bottom' }} />Shift schedule</h3>
          <p className="card-subtitle">Weekly working hours - employee, else club, else organization default.</p>
        </div>
      ...
      <div className="card-body">
        {canEdit && (
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 13.5, fontWeight: 600, marginBottom: 12 }}>
            ...
            Use a custom employee schedule
          </label>
        )}
            ...
            {canEdit && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
                <button type="button" className="btn btn-ghost btn-sm" disabled={busy}
                  onClick={() => { setDraft(staff.club_schedule); setErrors({}); }}>Copy from Club</button>
                <button type="button" className="btn btn-ghost btn-sm" disabled={busy}
                  onClick={() => { setDraft(staff.organization_schedule); setErrors({}); }}>Copy from Organization</button>
              </div>
            )}
            <ScheduleEditor
              scope="organization"
            ...
            {canEdit && (
              <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
                ...
                <button className="btn btn-secondary" disabled={busy}
                  onClick={() => { setDraft(staff.shift_hours); setErrors({}); }}>Reset</button>
              </div>
            )}
          </>
        ) : (
        ...
        <div className="row" style={{ marginTop: 16 }}>
          <div className="col"><ScheduleSummary title="Club schedule" week={staff.club_schedule} /></div>
          <div className="col"><ScheduleSummary title="Organization default" week={staff.organization_schedule} /></div>
        </div>
      </div>
    </div>
  );
...
function ScheduleSummary({ title, week }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '2px 12px', fontSize: 12.5 }}>
        {DAY_KEYS.map((d) => (
